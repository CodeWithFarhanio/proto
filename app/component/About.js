...
export default function About() {
  ...
  return (
    <ScrollReveal type="scale">
      <main className="min-h-screen bg-transparent text-black/90 py-[70px]">
          ...
          <div className="flex">
            ...
            <div>
              ...
              <div className="mb-8 flex items-center gap-5">
                <span className="h-px w-18 bg-[#aa0000] sm:w-18 md:w-36" />
                <span className="text-xs font-medium uppercase tracking-[0.35em] text-black/70">Who am I?</span>
              </div>
              ...
              <motion.h1 initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.4 }} className="mb-[60px] flex flex-col font-sans text-[clamp(65px,7vw,110px)] font-extrabold leading-[.82] tracking-[-6px] max-[650px]:mb-[40px] max-[650px]:text-[clamp(58px,17vw,90px)] max-[650px]:tracking-[-4px]">
                ...
                <div className="text-3xl font-semibold uppercase tracking-[0.01em] sm:text-6xl md:text-7xl lg:text-[6vw]">
                  ...
                  a passionate
                  <br />
                  <span className="text-[#aa0000]">Web Developer.</span>
                </div>
              ...
              <div className="mt-10 max-w-2xl ">
                ...
                <p className="text-sm font-extralight leading-7 text-black/70 sm:text-base">
                ...
                <div className="pointer-events-none absolute right-[5%] top-[18%] hidden h-[550px] w-[500px] lg:block">
                  {tech.map((item, index) => (
                    <motion.div key={item.name} animate={{ y: [0, -15, 0], rotate: [0, 8, 0] }} transition={{ duration: 3 + index * 0.3, repeat: Infinity, ease: "easeInOut" }} className={`pointer-events-none absolute flex h-[65px] w-[65px] select-none items-center justify-center rounded-[10px] font-sans text-[20px] font-extrabold shadow-[0_20px_50px_rgba(0,0,0,.5)] ${item.className}`}>
                      {item.name}
                    </motion.div>
                  ))}
                </div>
              ...
              {/* INFO GRID */}
              <div className="mt-12 grid grid-cols-2 gap-y-8 border-t border-black/10 pt-8 sm:grid-cols-4">

                {/* ROLE */}
                <div>
                  <p className="text-[10px] uppercase tracking-[0.25em] text-black/70">Role</p>
                  <p className="mt-2 text-sm uppercase">Full Stack</p>
                </div>

                {/* FOCUS */}
                <div>
                  <p className="text-[10px] uppercase tracking-[0.25em] text-black/70">Focus</p>
                  <p className="mt-2 text-sm uppercase">Web / Motion</p>
                </div>

                {/* STACK */}
                <div>
                  <p className="text-[10px] uppercase tracking-[0.25em] text-black/70">Stack</p>
                  <p className="mt-2 text-sm uppercase">Next.js</p>
                </div>

                {/* STATUS */}
                <div>
                  <p className="text-[10px] uppercase tracking-[0.25em] text-black/70">Status</p>
                  <p className="mt-2 text-sm uppercase text-[#aa0000]">Available</p>
                </div>
              ...
              <div className="mt-12 flex items-center gap-6">
                <button className="group flex items-center gap-4 border border-black/20 px-6 py-3 text-xs uppercase tracking-[0.2em] transition hover:border-[#aa0000] hover:text-[#aa0000]">
                  Explore More
